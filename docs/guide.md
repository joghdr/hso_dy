...
The current version of the code allows the user to
    ...
    - compute Hessian bands for the observables and eigensets
    - plot some results
...
pdfs, must be computed. In order to speed the code, these coefficients
...
directories:
    ...
    -MMHT2014nlo68cl/    (included for convenience, downloadable from official LHAPDF site)
...
1) Fits
  ...
  1.0) Running from script 'scripts/run_fit_fixed_target_DY.sh'
  ...
  See 'doc/compile_info.txt' for more information on 'scripts/compile.sh'
 ...
 1.5) Doing a fit with a custom model
 ...
 parts to modify are:
    ...
    a) the model for the CS kernel, with pQCD constraints:
    ...
    b) the 'core' model [1] for the TMDPDF
    ...
    c) the names of the parameters of both KtildeInputOrder_1 and FtildeCore
    ...
    d) the function minimized by Minuit

          double ROOT::Minuit2::FCN::operator()

    e) the input file for the parameters, in the same format as the sample one
       'input/para_hso.input', to include all of the names of step c
